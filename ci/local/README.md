...
Example Usage:
`bash build.sh -r ~/rapids/custrings -i gpuci/rapidsai-base:cuda9.2-ubuntu16.04-gcc5-py3.6`
...
Style Check:
```bash
$ bash ci/local/build.sh -r ~/rapids/custrings -s
...
The docker image will generate build artifacts in a folder on your machine located in the `root` directory of the repository you passed to the script. For the above example, the directory is named `~/rapids/custrings/build_rapidsai-base_cuda9.2-ubuntu16.04-gcc5-py3.6/`. Feel free to remove this directory after the script is finished.
