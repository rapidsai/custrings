...
int* indexes_that_match_this_string( NVStrings& strs, const char* str)
{
    ...
    int matches = thrust::count_if( thrust::device, d_indexes, d_indexes+count, []__device__(int x) { return x==0; });
    int* rtn = 0;
    cudaMalloc(&rtn,matches*sizeof(int));
    thrust::counting_iterator<int> itr(0);
    thrust::copy_if( thrust::device, itr, itr+count, rtn, [d_indexes] __device__(int i) { return d_indexes[i]==0; } );
    return rtn;
}
