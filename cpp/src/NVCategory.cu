...
NVCategory* NVCategory::add_keys_and_remap(NVStrings& strs)
{
    ...
    if( count==0 )
        return copy();
    // get the keys
    thrust::pair<const char*,size_t>* d_keys2 = 0;
    RMM_ALLOC(&d_keys2, count * sizeof(std::pair<const char*,size_t>),0);
    strs.create_index((std::pair<const char*,size_t>*)d_keys2);
    NVCategory* rtn = new NVCategory;
    if( kcount==0 )
    {
        // just take the keys; values are not effected
        // need to sort and unique them
        thrust::sort(execpol->on(0), d_keys2, d_keys2 + count,
            [] __device__( thrust::pair<const char*,size_t>& lhs, thrust::pair<const char*,size_t>& rhs ) {
                if( lhs.first==0 || rhs.first==0 )
                    return lhs.first==0; // non-null > null
                return custr::compare(lhs.first,(unsigned int)lhs.second,rhs.first,(unsigned int)rhs.second) < 0;
        ...
        auto nend = thrust::unique(execpol->on(0), d_keys2, d_keys2 + count,
            [] __device__ ( thrust::pair<const char*,size_t> lhs, thrust::pair<const char*,size_t> rhs ) {
                if( lhs.first==0 || rhs.first==0 )
                    return lhs.first==rhs.first;
                if( lhs.second != rhs.second )
        ...
        NVCategoryImpl_keys_from_index(rtn->pImpl,d_keys2,ucount);
        // copy the values
    ...
    rmm::device_vector<int> x(akcount);  // 0,1,....count2,-1,...,-count1
    int* d_x = x.data().get();
    // sequence and for-each-n could be combined into for-each-n logic
    thrust::sequence( execpol->on(0), d_x, d_x + kcount );   // first half is 0...count2
    thrust::for_each_n(execpol->on(0), thrust::make_counting_iterator<int>(0), count, [d_x, kcount] __device__ (int idx) { d_x[idx+kcount]= -idx-1; }); // 2nd half is -1...-count1
    ...
    NVCategoryImpl_keys_from_custringarray(pImpl,d_w,ucount);
    // remapping the values
    rmm::device_vector<int>* pNewMap = new rmm::device_vector<int>(mcount,0);
    int* d_map2 = pNewMap->data().get(); // first half is identical to map1
