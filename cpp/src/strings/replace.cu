...
struct replace_regex_fn
{
    ...
    __device__ void operator()(unsigned int idx)
    {
        ...
        if( mxn < 0 )
            mxn = (int)nchars; // max possible replaces for this string
...
NVStrings* NVStrings::replace_re( const char* pattern, const char* repl, int maxrepl )
{
    ...
    if( (regex_insts > MAX_STACK_INSTS) || (regex_insts <= 10) )
        thrust::for_each_n(execpol->on(0), thrust::make_counting_iterator<unsigned int>(0), count,
            replace_regex_fn<RX_STACK_SMALL>{prog, d_strings, d_repl, maxrepl, d_offsets, false, d_buffer, d_results});
    else if( regex_insts <= 100 )
        thrust::for_each_n(execpol->on(0), thrust::make_counting_iterator<unsigned int>(0), count,
            replace_regex_fn<RX_STACK_MEDIUM>{prog, d_strings, d_repl, maxrepl, d_offsets, false, d_buffer, d_results});
    else
        thrust::for_each_n(execpol->on(0), thrust::make_counting_iterator<unsigned int>(0), count,
            replace_regex_fn<RX_STACK_LARGE>{prog, d_strings, d_repl, maxrepl, d_offsets, false, d_buffer, d_results});
    //
    dreprog::destroy(prog);
    RMM_FREE(d_repl,0);
    return rtn;
