...
struct backrefs_fn
{
    ...
    __device__ void operator()(unsigned int idx)
    {
        ...
        else
        {
            ...
            d_results[idx] = custring_view::create_from(buffer,buffer,(unsigned int)(optr - buffer)); // new string
        }
    }
};
...
NVStrings* NVStrings::replace_with_backrefs( const char* pattern, const char* repl )
{
    ...
    if( (regex_insts > MAX_STACK_INSTS) || (regex_insts <= 10) )
        thrust::for_each_n(execpol->on(0), thrust::make_counting_iterator<unsigned int>(0), count,
            backrefs_fn<RX_STACK_SMALL>{prog, d_strings, d_repl, d_brefs, refcount, d_offsets, false, d_buffer, d_results});
    else if( regex_insts <= 100 )
        thrust::for_each_n(execpol->on(0), thrust::make_counting_iterator<unsigned int>(0), count,
            backrefs_fn<RX_STACK_MEDIUM>{prog, d_strings, d_repl, d_brefs, refcount, d_offsets, false, d_buffer, d_results});
    else
        thrust::for_each_n(execpol->on(0), thrust::make_counting_iterator<unsigned int>(0), count,
            backrefs_fn<RX_STACK_LARGE>{prog, d_strings, d_repl, d_brefs, refcount, d_offsets, false, d_buffer, d_results});
