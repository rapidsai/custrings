...
NVStrings* Rave::unique_tokens(NVStrings& strs, const char* delimiter )
{
    ...
    thrust::device_vector<custring_view*> strings(count,nullptr);
    ...
    thrust::device_vector< thrust::pair<const char*,size_t> > vocab;
    for( int col=0; col < columnsCount; ++col )
    {
        ...
        // sort the list
        thrust::sort(thrust::device, d_vocab, d_vocab + vocab.size(),
        ...
        // unique the list
        thrust::pair<const char*,size_t>* newend = thrust::unique(thrust::device, d_vocab, d_vocab + vocab.size(),
            [] __device__ ( thrust::pair<const char*,size_t> lhs, thrust::pair<const char*,size_t> rhs ) {
    ...
    thrust::pair<const char*,size_t>* d_vocab = vocab.data().get();
    auto end = thrust::remove_if(thrust::device, d_vocab, d_vocab + vocab.size(), [] __device__ ( thrust::pair<const char*,size_t> w ) { return w.first==0; } );
...
unsigned int Rave::token_count( NVStrings& strs, const char* delimiter, unsigned int* results, bool bdevmem )
{
    ...
    if( !bdevmem )
        cudaMalloc(&d_counts,count*sizeof(unsigned int));
...
unsigned int Rave::contains_strings( NVStrings& strs, NVStrings& tkns, bool* results, bool todevice )
{
    ...
    bool* d_rtn = results;
    if( !todevice )
        cudaMalloc(&d_rtn,tcount*count*sizeof(bool));
    
    //
    thrust::device_vector<custring_view*> strings(count,nullptr);
    thrust::device_vector<custring_view*> tokens(tcount,nullptr);
    custring_view** d_strings = strings.data().get();
...
unsigned int Rave::strings_counts( NVStrings& strs, NVStrings& tkns, unsigned int* results, bool todevice )
{
    ...
    if( !todevice )
        cudaMalloc(&d_rtn,tcount*count*sizeof(unsigned int));
